...
func main() {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "Failed to start FastWendy: %v\n", err)
		os.Exit(1)
	}
...
func newFastWendyECServer(conf *options, replicaConfig *config.ReplicaConfigFastWendy) *fastwendyecServer {
	...
	var pm hotstuff.PacemakerFastWendyEC
	switch conf.PmType {
	case "fixed":
		pm = pacemaker.NewFixedLeaderFastWendyEC(conf.LeaderID)
	case "round-robin":
		pm = pacemaker.NewRoundRobinFastWendyEC(
...
func (srv *fastwendyecServer) Start(address string) error {
...
func (srv *fastwendyecServer) Stop() {
	srv.gorumsSrv.Stop()
	srv.cancel()
	srv.wendyec.Close()
}

func (srv *fastwendyecServer) ExecCommand(_ context.Context, cmd *client.Command, out func(*client.Empty, error)) {
...
func (srv *fastwendyecServer) onExec() {
