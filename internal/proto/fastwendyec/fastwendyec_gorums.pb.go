...
// QuorumSpec is the interface of quorum functions for FastWendyEC.
type QuorumSpec interface {
}

// FastWendyEC is the server-side API for the FastWendyEC Service
type FastWendyEC interface {
	Propose(context.Context, *Block)
	Vote(context.Context, *PartialCert)
	NewView(context.Context, *NewViewMsg)
	Nack(context.Context, *NackMsg)
	ProofNoCommit(context.Context, *ProofNC)
}

func (s *GorumsServer) RegisterFastWendyECServer(srv FastWendyEC) {
	...
	s.srv.handlers[nackMethodID] = func(ctx context.Context, in *gorumsMessage, _ chan<- *gorumsMessage) {
		req := in.message.(*NackMsg)
		srv.Nack(ctx, req)
	}
	s.srv.handlers[proofNoCommitMethodID] = func(ctx context.Context, in *gorumsMessage, _ chan<- *gorumsMessage) {
		req := in.message.(*ProofNC)
...
var orderingMethods = map[int32]methodInfo{
	...
	2: {requestType: new(NewViewMsg).ProtoReflect(), responseType: new(emptypb.Empty).ProtoReflect()},
	3: {requestType: new(NackMsg).ProtoReflect(), responseType: new(emptypb.Empty).ProtoReflect()},
	4: {requestType: new(ProofNC).ProtoReflect(), responseType: new(emptypb.Empty).ProtoReflect()},
}
...
func (n *Node) NewView(in *NewViewMsg) error {
	msgID := n.nextMsgID()
	metadata := &ordering.Metadata{
		MessageID: msgID,
...
func (n *Node) Nack(in *NackMsg) error {
	msgID := n.nextMsgID()
	metadata := &ordering.Metadata{
		MessageID: msgID,
		MethodID:  nackMethodID,
	}
	msg := &gorumsMessage{metadata: metadata, message: in}
	n.sendQ <- msg
	return nil
...
func (n *Node) ProofNoCommit(in *ProofNC) error {
	msgID := n.nextMsgID()
	metadata := &ordering.Metadata{
		MessageID: msgID,
