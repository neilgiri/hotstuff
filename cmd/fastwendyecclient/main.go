...
func main() {
	...
	client, err := newFastWendyECClient(&conf, replicaConfig)
	if err != nil {
	...
	if !conf.Benchmark {
		...
		for _, cmd := range commandStats {
			latency := float64(cmd.GetDuration().GetNanos())
			commandLatency := latency / float64(time.Millisecond)
...
func newFastWendyECClient(conf *options, replicaConfig *config.ReplicaConfig) (*fastwendyecClient, error) {
...
func (c *fastwendyecClient) Close() {
	c.mgr.Close()
	c.reader.Close()
}

func (c *fastwendyecClient) GetStats() *benchmark.Result {
	return c.stats.GetResult()
}

func (c *fastwendyecClient) SendCommands(ctx context.Context) error {
	var num uint64
