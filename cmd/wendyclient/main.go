...
func main() {
	...
	replicaConfig := config.NewConfigBls(0, nil, nil, nil)
	for _, r := range conf.Replicas {
		...
		replicaConfig.Replicas[r.ID] = &config.ReplicaInfoBls{
			ID:      r.ID,
	...
	client, err := newWendyClient(&conf, replicaConfig)
	if err != nil {
...
type wendyClient struct {
	...
	conf          *options
	mgr           *client.Manager
	replicaConfig *config.ReplicaConfigBls
...
func newWendyClient(conf *options, replicaConfig *config.ReplicaConfigBls) (*wendyClient, error) {
...
func (c *wendyClient) Close() {
	c.mgr.Close()
	c.reader.Close()
}

func (c *wendyClient) GetStats() *benchmark.Result {
	return c.stats.GetResult()
}

func (c *wendyClient) SendCommands(ctx context.Context) error {
	var num uint64
