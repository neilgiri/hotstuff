...
func (p *FixedLeaderFastWendyEC) Run(ctx context.Context) {
...
func (p *RoundRobinFastWendyEC) GetLeader(view int) config.ReplicaID {
	term := int(math.Ceil(float64(view) / float64(p.termLength)))
	return p.schedule[term%len(p.schedule)]
}

// Run runs the pacemaker which will beat when the previous QC is completed
func (p *RoundRobinFastWendyEC) Run(ctx context.Context) {
	// initial beat
...
func (p *RoundRobinFastWendyEC) startNewViewTimeout(stopContext context.Context) {
	for {
		select {
		case <-p.resetTimer:
