...
func usage() {
	...
	fmt.Println()
	fmt.Println("Loads configuration from ./wendy.toml and file specified by --config")
	fmt.Println()
	fmt.Println("Options:")
	pflag.PrintDefaults()
}
...
func main() {
	...
	privkeyBls := data.GeneratePrivateKeyBls()
	...
	replicaConfig := config.NewConfigBls(conf.SelfID, &privkeyBls, cert, privkey)
	...
	srv := newWendyServer(&conf, replicaConfig)
...
func newWendyServer(conf *options, replicaConfig *config.ReplicaConfigBls) *wendyServer {
	...
	var pm hotstuff.PacemakerWendy
	switch conf.PmType {
	case "fixed":
		pm = pacemaker.NewFixedLeaderWendy(conf.LeaderID)
	case "round-robin":
		pm = pacemaker.NewRoundRobinBls(
	...
	srv.wendy = hotstuff.NewWendy(replicaConfig, pm, conf.TLS, time.Minute, time.Duration(conf.ViewTimeout)*time.Millisecond)
...
func (srv *wendyServer) Start(address string) error {
...
func (srv *wendyServer) Stop() {
	srv.gorumsSrv.Stop()
	srv.cancel()
	srv.wendy.Close()
}

func (srv *wendyServer) ExecCommand(_ context.Context, cmd *client.Command, out func(*client.Empty, error)) {
	...
	srv.wendy.AddCommand(data.Command(b))
...
func (srv *wendyServer) onExec() {
	for cmds := range srv.wendy.GetExec() {
